...
export default function EditEvent({ params }: { params: Promise<{ id: string }> }) {
    ...
    const fetchEventData = async () => {
        try {
            const event = await eventsService.getEventById(parseInt(resolvedParams.id))
            setFormData({
                title: event.title,
                type: event.type,
    ...
    const handleSubmit = async () => {
        try {
            ...
            await eventsService.updateEvent(parseInt(resolvedParams.id), eventData)
        ...
        <div className="container mx-auto py-6 px-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 bg-gradient-to-r from-[#074A29] to-[#1A9A32] p-4 rounded-lg text-white">
                <div>
                    ...
                    <p className="text-white/80">Update your event details, schedule, and more</p>
                </div>
            </div>
            ...
            {currentStep === 4 && (
                <Card className="bg-black border-[#1A9A32]/30 mb-6">
                    <CardHeader>
                        <CardTitle className="text-white text-xl">Review & Update</CardTitle>
                        <CardDescription className="text-white/70">Review your event details before updating</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-6">
                        <Alert className="bg-[#074A29]/20 border-[#1A9A32]/50 text-white">
                            <AlertCircle className="h-4 w-4 text-[#B6E29F]" />
                            <AlertTitle>Almost there!</AlertTitle>
                            <AlertDescription>Please review all the information below before updating your event.</AlertDescription>
                        </Alert>
